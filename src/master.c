...
static void *network_thread(void *arg) {
    ...
    if (na->is_server) {
        ...
        send_line(sock, buf);
        if (recv_line(sock, buf, sizeof(buf)) <= 0) goto done; // sok
    } else {
        if (recv_line(sock, buf, sizeof(buf)) <= 0) goto done; // ok
        send_line(sock, "ook\n");
        if (recv_line(sock, buf, sizeof(buf)) <= 0) goto done; // size w h
        int w = 0, h = 0;
    ...
    while (1) {
        if (na->is_server) {
            ...
            if (st == 2) {
                send_line(sock, "q\n");
                recv_line(sock, buf, sizeof(buf)); // qok
                break;
            }

            // Send drone position
            send_line(sock, "drone\n");
            int vx, vy;
            to_virtual_coords(w, h, x, y, &vx, &vy);
            snprintf(buf, sizeof(buf), "%d %d\n", vx, vy);
            send_line(sock, buf);
            if (recv_line(sock, buf, sizeof(buf)) <= 0) break; // dok

            // Receive obstacle (client's drone)
            send_line(sock, "obst\n");
            if (recv_line(sock, buf, sizeof(buf)) <= 0) break; // x y
            int ovx, ovy;
            ...
            if (strcmp(buf, "drone") == 0) {
                ...
                snprintf(buf, sizeof(buf), "%d %d\n", vx, vy);
                send_line(sock, buf);
                recv_line(sock, buf, sizeof(buf)); // pok
            }
        }
